...
- A validated file can now be handed directly to Bambu Studio or an installed
  OrcaSlicer. The Orca option is a convenient file handoff, not a slicer plugin;
  Bambu-specific mixed-color validation remains a Bambu Studio check.
...
These viewer screenshots are from a detail run for showing the modes; palette
counts can differ from a separate balanced benchmark run.

Original plate view:

![FullSpectrum Studio original view](https://github.com/Gr33k3D/FullSpectrum-Studio/blob/main/teasers/v0.4.3-original.png?raw=true)
...
![FullSpectrum Studio validation view](https://github.com/Gr33k3D/FullSpectrum-Studio/blob/main/teasers/v0.4.3-validation.png?raw=true)
...
experimental. Very large source models use an optimized movable preview to
keep the app responsive while the full archive is still used for conversion
and validation.
