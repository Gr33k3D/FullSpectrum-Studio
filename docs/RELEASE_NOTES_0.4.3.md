...
- Shareable text reports omit local inventory quantities and reference
  filenames.
- macOS builds are re-signed after assembly and must pass strict bundle
  verification before release packaging.

## Slicer Handoff

- Validated files can be opened in installed Bambu Studio or OrcaSlicer from
  the desktop app.
- OrcaSlicer support is a saved-file handoff, not a plugin or claimed schema
  extension. Bambu loaded-swatch validation remains authoritative only for
  Bambu Studio workflows.
- Tagged releases build tested macOS and Windows artifacts from source and
  attach them to the prerelease rather than storing local binaries in the
  repository tree.
